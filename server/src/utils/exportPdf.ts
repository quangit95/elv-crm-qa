...
export function generateQuotationPDF(quotation: any, company: any, res: Response) {
  // A4 size: 595.28 x 841.89
  const doc = new PDFDocument({ margin: 30, size: 'A4' })

  res.setHeader('Content-Type', 'application/pdf')
  res.setHeader('Content-Disposition', `inline; filename=Quotation-${quotation.code}.pdf`)
  ...
  // Register fonts if needed or just use built-in if we have issues but Windows usually has these
  try {
      doc.font(fontNormal)
  } catch (e) {
